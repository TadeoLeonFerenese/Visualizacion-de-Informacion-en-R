...
sube %>%
  ...
  geom_col(width = 20) +
  labs(x = "Periodo", y = "Cantidad", fill = "Modo de transporte") +
  theme_minimal()+
  theme(axis.text.y = element_blank())+
  facet_wrap()
```

## CORRECCION A LA PREGUNTA 1 parte 1 - OPCION 1
```{r}
sube %>% 
  ggplot(aes(periodo, fill = modo))+
  geom_histogram(color = "white", bins = 30)+
  labs( x = "Periodo", y = "Cantidad", fill = "Modo de transporte")+
  theme_minimal()
```

## CORRECCION A LA PREGUNTA 1 parte 1 - OPCION 2 

```{r}
sube %>% 
  ggplot(aes( periodo))+
  geom_density(aes(fill = modo))+
  labs( x = "Periodo", y = "Cantidad", fill = "Modo de transporte")+
  facet_wrap(~ modo, scales = "free_x")+ # EL FACE WRAP LO AGREGUE POR QUE SINO NO SE PUEDE LEER BIEN CARA UNA DE LAS AREAS, PERO AUN ASI SE VE QUE PREODIMNA EL BONDI
  theme_minimal()+
  scale_y_continuous(labels = scales::number)
```

### Operaciones en colectivo con distintos pasajes (Correcion para el recuperatorio OPCION 1)
...
ggplot(data = sube_filtrado, aes(y = tipo_pasaje, x = suma_de_cantidad, fill = modo)) +
  ...
  labs(x = "Tipo de Pasaje", y = "Cantidad", fill = "Tipo de Pasaje") +
  theme_minimal()+
...
sube_empresas <- sube %>% 
  group_by(empresa, grupo_tarifario) %>% 
  summarise(cantidad = sum(empresa))
```
